...
describe('Starting tests for land-agreement', function() {
  ...
  it('website title should be land-agreement', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('land-agreement');
    })
  });

  it('network-name should be land-agreement@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('land-agreement@0.0.1.bna');
    });
  });

  it('navbar-brand should be land-agreement',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('land-agreement');
    });
  });
    ...
    it('Land table should have 5 columns',() => {
      page.navigateTo('/Land');
